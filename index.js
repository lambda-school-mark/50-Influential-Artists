...
function getArtistByIndex(inventory, name) {
  for (let i = 0; i < artists.length; i++) {
    if (inventory[i].name === name) {
      return `The artist at ${inventory[i].id} is ${inventory[i].name}.`;
    }
  }
}
console.log(getArtistByIndex(artists, "Vincent Van Gogh"));
/**
...
function removeArtist(name, id) {
  for (let i = 0; i < artists.length; i++) {
    if (artists[i].name === name && artists[i].id === id) artists.splice(i, 1);
  }
}
...
addArtist(
  ...
  "Web Design",
  "American",
  "that one kid learning JavaScript"
);
console.log(artists);
